...
public class RayOfFrostAbility implements Ability{
	int time = 6;
	int damage = 2;
	int range = 20;
	...
	public List<String> getDescription() {
		// TODO Auto-generated method stub
		return Arrays.asList("Instantly fire a frosty ray up to " + range + " blocks,",
		...
		se3.visibleRange = 200;
			...
			for(int i = 0; i < range+5 ; i++)
			{
				...
				{	
					...
					AbilityUtils.addPotionDuration(p, (LivingEntity) e, "Frost Ray", PotionEffectType.SLOW, 1, time*20);
                 	 p.getWorld().playSound(loc, Sound.ENTITY_SNOWMAN_HURT, 2, 0.6F);
					players = true;	
					}
				}
				if(players)
